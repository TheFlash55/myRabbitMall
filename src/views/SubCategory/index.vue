...
const load = async () => {
  reqData.value.page++;
  console.log("reqData.value.page", reqData.value.page);
  const res = await getSubCategoryAPI(reqData.value); //这是拿到的下一页的数据，需要进行合并
  goodList.value = [...goodList.value, ...res.result.items]; //合并旧数据和新加载的数据用于展示
  //接下来进行判断，没数据可加载则将disabled置为true
